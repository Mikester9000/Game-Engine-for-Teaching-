...
| **ComboSystem (M19)** | ✅ | `src/engine/combat/combo_system.hpp/.cpp` — IDLE/BUILDING/COOLDOWN FSM; `ComboDefinition` + `CombatConfig`; `LoadConfig()` (ENGINE_ENABLE_JSON); `AddCombo()` direct API; prefix matching + exact match + smart reset (plinking); `--scene combat_test` 4-test headless CI |
...
**M3 audio is ✅ complete. M3 D3D11 textured quad is ✅ complete.** M4b (IK solver + D3D11 GPU skinning) is ✅ complete. M5 (Jolt Physics) is ✅ complete. M6 (Editor) is ✅ complete. M7 (World Streaming, all sub-milestones M7.1–M7.5) is ✅ complete. M8 (Gameplay Integration, all sub-milestones M8.1–M8.9) is ✅ complete. **M10 (Dynamic Sky + weather VFX, all sub-milestones M10.1–M10.7) is ✅ complete.** **Post-M10 Vehicle Physics is ✅ complete.** **Post-M10 Behaviour Tree AI is ✅ complete:** `BtTree`/`BtSequence`/`BtSelector`/`BtCondition`/`BtAction`/`BtBlackboard` (engine/ai/behaviour_tree); `FormationSystem` (LINE/V_SHAPE/CIRCLE slot layouts, greedy assignment, world-space transform); grid `NavMesh` (BakeFromGrid, A* FindPath, obstacle routing); `--scene bt_test` 4-test headless CI. **Post-M10 SDF Font Renderer is ✅ complete:** `FontRenderer` (SDF atlas, R8_UNORM D3D11 texture, sdf_text.vs/ps.hlsl, `--scene font_test` CI). **`pak.exe` PAK1 packager is ✅ complete:** `src/tools/pak/pak_main.cpp` (PAK1 format, `--input`/`--list`/`--extract`, CI test). **Post-M10 Cinematics runtime is ✅ complete:** `CinematicSequencer` + `CameraRig` + `CameraComponent.cinematicOverride` + `--scene cinematic_test` 4-test headless CI (incl. timed audio events). **M22 (Cut-scene baker + cinematic editor panel) is ✅ complete:** `shared/schemas/cinematic.schema.json`; `tools/creation_engine.py` `bake-cinematic` (`.cinematic.json` → `.cinematic`); `editor/src/panels/CinematicEditorPanel.hpp/.cpp` timeline + keyframe authoring + editor preview controls; sample `Content/Cinematics/intro.cinematic.json`; baker pytest coverage. **M16 (D3D11 depth buffer + IBL) is ✅ complete:** D24_UNORM_S8_UINT depth buffer wired into DrawFrame; CPU-generated BRDF LUT + irradiance cubemap + prefiltered env cubemap (Hammersley, GGX importance sampling); `pbr_ibl.vs/ps.hlsl`; `--scene pbr_ibl` 4-test headless CI. **M17 (D3D11 shadow maps + bloom) is ✅ complete:** directional shadow map (512×512 D32_FLOAT, depth-bias rasterizer, 3×3 PCF via `SamplerComparisonState`, `shadow.vs.hlsl` + `shadow_lit.vs/ps.hlsl`); LDR bloom demo (4× 256×256 RGBA8 offscreen RTs, 5-tap separable Gaussian blur ping-pong, Reinhard composite, `bloom_bright/blur/composite.ps.hlsl`); `--headless --scene shadow_test` + `bloom_test` 3-test CI each. **M18 (X3DAudio 3D positional audio) is ✅ complete:** `X3DAudioInitialize` after mastering voice; `SetListenerPosition`/`Compute3DVolume`/`Apply3DAttenuation` on `XAudio2Backend` (X3DAudio DSP path + linear fallback); per-frame attenuation in `AudioSystem::Update` reads `TransformComponent`; `--headless --scene audio_3d_test` 3-test CI. **M19 (Action combat completion) is ✅ complete:** `ComboSystem` FSM (IDLE/BUILDING/COOLDOWN); `ComboDefinition` + `CombatConfig`; `LoadConfig()` (ENGINE_ENABLE_JSON gated); `combat_config.json` (4 combos: Avalanche Chain, Warp Strike, Magic Burst, Phantom Sword); `--headless --scene combat_test` 4-test CI (combo_populate, combo_sequence, window_expiry, damage_formula). **M20 (Quest/dialogue tools + tests) is ✅ complete:** `tools/quest_baker/` Python tool (QuestBaker + DialogueBaker, 15 pytest); `shared/schemas/quest_bank.schema.json` + `dialogue_tree.schema.json`; sample `quest_bank.json` + `dialogue_tree.json` in vertical_slice_project; `--headless --scene quest_test` 4-test CI (quest_accept, quest_objective, quest_prereq, quest_fail); `--headless --scene dialogue_test` 3-test CI (out_of_range, in_range, begin_and_advance). **M21 (Tool stubs: nav-mesh baker + tod.lut baker) is ✅ complete:** `tools/creation_engine.py` now provides `bake-navmesh` (`.obj` → `.navmesh`) and `bake-tod` (`tod.json` → `tod.lut`) commands plus pytest coverage in `tools/tests/test_creation_engine_bakers.py` and Linux CI wiring. **M27 (Extra bakers + cinematic audio events) is ✅ complete:** `bake-collision` (OBJ → PHY1), `bake-font` (font.json → FNT1), `bake-road` (road.json → RD01) bakers added to `creation_engine.py`; 18 pytest; Linux CI smoke steps; `CinematicSequencer::AddAudioEvent` + `SetOnAudioEvent` timed dispatch; 4th `cinematic_test` sub-test (audio_event_early, audio_event_fired, audio_event_once); `asset_registry.schema.json` extended with `collision` + `road` types; sample assets committed. Next: M14 Vulkan catch-up (deferred).
...
| Cinematic baker tool | ✅ | `tools/creation_engine.py` `bake-cinematic` — timeline JSON → cooked `.cinematic` (M22) |
| Cinematic editor panel | ✅ | `editor/src/panels/CinematicEditorPanel.hpp/.cpp` — shot timeline, keyframe/property edit, preview controls (M22) |
...
| ~~**1 — Now**~~ | ~~**M19: Action combat completion**~~ | ✅ done — `ComboSystem` FSM (IDLE/BUILDING/COOLDOWN); `ComboDefinition` + `CombatConfig`; `LoadConfig()` (ENGINE_ENABLE_JSON gated); `combat_config.json` (4 combos); `--headless --scene combat_test` 4-test CI (combo_populate, combo_sequence, window_expiry, damage_formula) |
| ~~**1 — Now**~~ | ~~**M20: Quest/dialogue tools + tests**~~ | ✅ done — `tools/quest_baker/` (QuestBaker + DialogueBaker, 15 pytest); `quest_bank.schema.json` + `dialogue_tree.schema.json`; sample `quest_bank.json` + `dialogue_tree.json`; `--headless --scene quest_test` 4-test CI; `--headless --scene dialogue_test` 3-test CI |
| ~~**1 — Now**~~ | ~~**M21: Tool stubs**~~ | ✅ done — `creation_engine.py` `bake-navmesh` + `bake-tod` + pytest + Linux CI step |
| ~~**1 — Now**~~ | ~~**M22: Cinematic baker + editor panel**~~ | ✅ done — `shared/schemas/cinematic.schema.json`, `creation_engine.py bake-cinematic`, `CinematicEditorPanel` |
| ~~**1 — Now**~~ | ~~**M27: Extra bakers + cinematic audio events**~~ | ✅ done — `bake-collision` (PHY1), `bake-font` (FNT1), `bake-road` (RD01); 18 new pytest; `AddAudioEvent`/`SetOnAudioEvent` on CinematicSequencer; 4th cinematic_test sub-test; schema extended |
...
| M27 | Extra bakers (PHY1/FNT1/RD01) + cinematic audio events | ✅ |
