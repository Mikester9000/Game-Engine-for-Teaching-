...
#include <nlohmann/json.hpp>  // cells_manifest.json validation in --validate-project
#endif
...
#  define ENGINE_VERSION_SHA "unknown"
        ...
        if (!validateProject.empty())
        {
            ...
            // -------------------------------------------------------------------
            // TEACHING NOTE — Validate cells_manifest.json (streaming cell GUIDs)
            // -------------------------------------------------------------------
            // cells_manifest.json is generated by cook.exe alongside assetdb.json.
            // It lists every level cell with its (cx,cz) coordinates and asset GUID.
            // We validate:
            //   1. The file exists and parses as JSON.
            //   2. Each cell entry has cx/cz/guid fields.
            //   3. Each GUID exists in the AssetDB.
            //   4. Each GUID's cooked file is present on disk.
            // If the manifest is absent it is a soft warning (cook may not have run).
            // If it is present but malformed or references missing assets → [FAIL].
            // -------------------------------------------------------------------
            {
                const fs::path manifestPath =
                    projectPath / "Cooked" / "Levels" / "cells_manifest.json";

                if (!fs::exists(manifestPath))
                {
                    std::cout << "[validate-project] cells_manifest.json not found at "
                              << manifestPath.string()
                              << " — run cook.exe --project <projectPath> to generate it.\n";
                    std::cout << "  Hint: cmake --build <preset> --target cook_samples\n";
                }
                else
                {
#ifdef ENGINE_ENABLE_JSON
                    ...
                    try
                    {
                        ...
                        if (cellErrors > 0)
                        {
                            std::cout << "[FAIL] cells_manifest.json: "
                                      << cellErrors << " error(s) in "
                                      << cellCount  << " cell(s).\n";
                            return 1;
                        }
                        std::cout << "[PASS] cells_manifest.json validated: "
                                  << cellCount << " cell(s) OK.\n";
                    }
                    catch (const std::exception& ex)
                    {
                        std::cout << "[FAIL] cells_manifest.json parse error: "
                                  << ex.what() << "\n";
                        manifestOk = false;
                    }
                    if (!manifestOk) return 1;
#else
                    std::cout << "[validate-project] cells_manifest.json found but "
                                 "ENGINE_ENABLE_JSON not defined — skipping deep validation.\n";
        ...
        if (headless)
        {
            ...
            else if (scene == "m8_streaming")
            {
                ...
                // 3. Load cells_manifest.json to register cell GUIDs.
                // TEACHING NOTE — Data-driven GUID registration in tests
                // ─────────────────────────────────────────────────────────
                // The cells_manifest.json is generated by cook.exe and maps
                // each world-streaming cell (cx, cz) to its stable asset GUID.
                // We parse it with a minimal hand-written scanner so this test
                // has no nlohmann/json dependency and runs in every build config.
                //
                // If the manifest is absent or cannot be parsed, we fall back to
                // registering no GUIDs.  Cells still reach LOADED state via the
                // procedural fallback in GameStreamingManager::OnLoadCell().
                {
                    const std::string kManifestPath =
                        "samples/vertical_slice_project/Cooked/Levels/cells_manifest.json";

                    std::ifstream mf(kManifestPath);
                    if (!mf.is_open())
                    {
                        std::cout << "[INFO] m8_streaming: cells_manifest.json not found — "
                                     "registering no cell GUIDs (procedural fallback).\n";
                    }
                    else
                    {
                        // Minimal scanner: read the whole file and extract
                        // "cx", "cz", "guid" fields from each object entry.
                        const std::string content(
                            (std::istreambuf_iterator<char>(mf)),
                             std::istreambuf_iterator<char>());

                        // Find every "guid" field and back-track for cx/cz.
                        std::size_t pos = 0;
                        while (pos < content.size())
                        {
                            // Locate the next cell object by finding "cx".
                            const std::string kCxKey = "\"cx\"";
                            ...
                            // Helper: read string value after ':' from pos.
                            auto readStr = [&](std::size_t from, std::string& out) -> std::size_t
                            {
                                std::size_t p = content.find(':', from);
                                if (p == std::string::npos) return std::string::npos;
                                p = content.find('"', p + 1);
                                if (p == std::string::npos) return std::string::npos;
                                ++p;
                                std::size_t end = content.find('"', p);
                                if (end == std::string::npos) return std::string::npos;
                                out = content.substr(p, end - p);
                                return end + 1;
                            };

                            int cx = 0, cz = 0;
                            std::size_t after = readInt(cxPos + kCxKey.size(), cx);
                            if (after == std::string::npos) { pos = cxPos + 1; continue; }

                            const std::string kCzKey = "\"cz\"";
                            std::size_t czPos = content.find(kCzKey, after);
                            if (czPos == std::string::npos) break;
                            after = readInt(czPos + kCzKey.size(), cz);
                            if (after == std::string::npos) { pos = cxPos + 1; continue; }

                            const std::string kGuidKey = "\"guid\"";
                            std::size_t guidPos = content.find(kGuidKey, after);
                            if (guidPos == std::string::npos) break;
                            std::string guid;
                            after = readStr(guidPos + kGuidKey.size(), guid);
                            if (after == std::string::npos) { pos = cxPos + 1; continue; }

                            const uint32_t cellId =
                                engine::world::CellIdFromCoord({ cx, cz });
                            streamMgr->RegisterCellGuid(cellId, guid);
                            pos = after;
                        }
                    }
                }
